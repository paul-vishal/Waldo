...
def formatUserPreference(prefObj):
    for x in prefObj:
        userDataObj = [x['photography'], x['sports'], x['tattoo'], x['games']]
    return userDataObj
...
def constructDataForNotification(user, apiData):
    ...
    for i in range(0, len(user)):
        for j in range(0, 4):
            if user[i][1][j]:
                if j == 0:
                    userDataObj.append([user[i][0], apiData['photography']])
                elif j == 1:
                    userDataObj.append([user[i][0], apiData['sports']])
                elif j == 2:
                    userDataObj.append([user[i][0], apiData['tattoo']])
                elif j == 3:
                    userDataObj.append([user[i][0], apiData['games']])
    return userDataObj


consumer = KafkaConsumer(
    'cars', 'digitalart', 'minimalism',
