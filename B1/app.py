...
def formatUserPreference(prefObj):
    for x in prefObj:
        userDataObj = [x['anime'], x['abstract'], x['animals'], x['cars']]
    return userDataObj
...
def constructDataForNotification(user, apiData):
    ...
    for i in range(0, len(user)):
        for j in range(0, 4):
            if user[i][1][j]:
                if j == 0:
                    userDataObj.append([user[i][0], apiData['anime']])
                elif j == 1:
                    userDataObj.append([user[i][0], apiData['abstract']])
                elif j == 2:
                    userDataObj.append([user[i][0], apiData['animals']])
                elif j == 3:
                    userDataObj.append([user[i][0], apiData['cars']])
    return userDataObj


consumer = KafkaConsumer(
    'anime', 'tattoo', 'videogames',
